...
void Load(const int r, Operand* opd)	//操作数opd加载到寄存器r中
{
	...
	else
	{
		...
		{
			GenOpcode_1(0x8d);
			GenOpcode_2(ADDR_OTHER, r, ViaC_LOCAL, opd->sym, fc);
		}
		else if (v == ViaC_CMP)
		{
			GenOpcode_1(0xb8 + r);
...
void Store_1(void) //将栈顶的操作数存入次栈顶的操作中
{
	...
	if ((optop[-1].reg & ViaC_VALMASK) == ViaC_LLOACL)
	{
		Operand opd;
...
void SpillReg(int reg) //将‘r’溢出到内存栈中，并且标记释放‘r’寄存器的操作数
{
	...
	for (popd = opstack; popd <= optop; ++popd)
	{
		if ((popd->reg & ViaC_VALMASK) == reg)
		{
			...
			if (popd->reg & ViaC_LVAL)
			{
				popd->reg = (popd->reg &  ~(ViaC_VALMASK)) | ViaC_LLOACL;
			}
			else
			{
...
void InitVariable(const Type* ptype, const Section* psec, const int c, const int v) //变量初始化
{
	int bt;
	void* ptr;
	...
	if (psec)
	{
		...
		switch (bt)
		{
			...
			default:
			{
				...
				{
					CoffElocAdd(psec, optop->sym, c, IMAGE_REL_I386_DIR32);
				}
				*(int*)ptr = optop->value;
				break;
			}
