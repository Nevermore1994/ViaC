...
void Load(int r, Operand *opd)
{
	...
	else
	{
		...
		else if (v == SC_LOCAL)
		{
			...
			GenOpcode_1(0x8d);
			GenModrm(ADDR_OTHER, r, SC_LOCAL, opd->sym, fc);
		}
...
void Store_1()
{
	...
	if ((optop[-1].reg & SC_VALMASK) == SC_LLOACL)
	{
		...
		OperandAssign(&opd, T_INT, SC_LOCAL | SC_LVAL, optop[-1].value);
		Load(t, &opd);
...
{
	int r;
	if ((optop->reg & (SC_VALMASK | SC_LVAL)) == SC_GLOBAL)
	{
		// 记录重定位信息
...
* op:		运算符类型
**********************************************************/
void GenOpi_2(int opc, int op)
{
	int r, fr, c;
	if ((optop->reg & (SC_VALMASK | SC_LVAL | SC_SYM)) == SC_GLOBAL)
	{
...
void GenOp(int op)
{
	...
	if (bt1 == T_PTR || bt2 == T_PTR)
	{
		...
		else //两个操作数一个是指针，另一个不是指针，并且非关系运算
		{
			...
			type1 = optop[-1].type;
			OperandPush(&int_type, SC_GLOBAL, PointedSize(&optop[-1].type));
			GenOp(TK_STAR);
...
void SpillReg(int r)
{
	...
	for (p = opstack; p <= optop; p++)
	{
		if ((p->reg & SC_VALMASK) == r)
		{
			...
			OperandAssign(&opd, type->t, SC_LOCAL | SC_LVAL, loc);
			Store(r, &opd);
			if (p->reg & SC_LVAL)
			{
				p->reg = (p->reg & ~(SC_VALMASK)) | SC_LLOACL; //标识操作数放在栈中
			}
			else //sum = add(a = add(a,b),b = add(c,d));
			{
				p->reg = SC_LOCAL | SC_LVAL;
			}
			p->value = loc;
			break;
		}
...
void InitVariable(Type *type, Section *sec, int c, int v)
{
	...
	if (sec)
	{
		...
		switch (bt)
		{
			...
			default:
				if (optop->reg & SC_SYM)
				{
					CoffElocAdd(sec, optop->sym, c, IMAGE_REL_I386_DIR32);	// 适用于char *g_pstr = "g_pstr_Hello";
				}
				*(int *)ptr = optop->value;
				break;
		}
	...
	else
	{
		if (type->t&T_ARRAY)
		{
			OperandPush(type, SC_LOCAL | SC_LVAL, c);
			OperandSwap();
			SpillReg(REG_ECX);
			...
			// 8D /r	LEA r32,m	Store effective address for m in register r32
			GenOpcode_1(0x8D);
			GenModrm(ADDR_OTHER, REG_EDI, SC_LOCAL, optop->sym, optop->value); // lea edi, [ebp-n]
...
Section *AllocateStorage(Type *type, int r, int has_init, int v, int *addr)
{
	...
	else
	{
		...
		*addr = sec->data_offset;
		...
		if (v == 0) //常量字符串
		{
			OperandPush(type, SC_GLOBAL | SC_SYM, *addr);
